...
void main() {
  ...
  group('TransactionController |', () {
    ...
    final expectedMonthYear = "${tDate.month.toString().padLeft(2, '0')}${tDate.year}";

    test('Deve chamar createTransaction no repositorio ao salvar uma nova transaçao', () async {
      ...
      final captured = verify(() => mockRepository.createTransaction(captureAny())).captured;
    ...
    test('Deve chamar deleteTransaction no repositorio com o ID correto', () async {
      when(() => mockRepository.deleteTransaction('id_123')).thenAnswer((_) async {});
    ...
    test('Deve atualizar uma transacao preservando o ID e a data de criacao original', () async {
      final tDate = DateTime.now();
      ...
      expect(updatedEntity.id, 'id_123', reason: 'O ID não pode ser alterado');
      expect(updatedEntity.createdAt, oldDate, reason: 'A data de criação original deve ser preservada');
      expect(updatedEntity.updatedAt.isAfter(oldDate), isTrue, reason: 'A data de atualização deve ser recente');
      expect(updatedEntity.monthYear, expectedMonthYear, reason: 'O mês/ano deve ser no formato MMYYYY');
    });
    ...
    test('Deve retornar uma lista de transacoes ao buscar por Mes/Ano', () async {
      final dummyList = [FakeTransactionEntity(), FakeTransactionEntity()];
      when(() => mockRepository.getTransactionsByMonthYear('032026'))
      ...
      verify(() => mockRepository.getTransactionsByMonthYear('032026')).called(1);
    });

    test('Deve retornar uma lista de transacoes ao buscar por Filtro', () async {
      final dummyList = [FakeTransactionEntity()];
      final filter = TransactionFilter(isFixed: true);

      when(() => mockRepository.getTransactionsByFilter(filter))
          .thenAnswer((_) async => dummyList);
