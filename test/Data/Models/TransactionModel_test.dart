...
void main() {
  group('TransactionModel |', () {
    ...
    test('Should convert the model to a SQLite-compatible Map (toMap)', () {
      ...
      expect(result['isFixed'], 1, reason: 'The boolean true must be converted to 1');
      expect(result['isPaid'], 0, reason: 'The boolean false must be converted to 0');
      expect(result['transactionDate'], tDate.toIso8601String(), reason: 'The date must be in ISO8601 format');
    });

    test('Should convert a SQLite Map (fromMap) back into a TransactionModel', () {
          ...
          expect(result.isFixed, false, reason: 'The value 0 from the database must become false');
          expect(result.isPaid, true, reason: 'The value 1 from the database must become true');
